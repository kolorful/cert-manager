...
func (c *controller) Sync(ctx context.Context, ch *cmacme.Challenge) (err error) {
	...
	// if DNS01 check recreation is enabled and the pending challenge's creation time is DNS01CheckRecreatePeriod ealier than now
	// delete the stale challenge so it can be recreated
	if rp := c.DNS01CheckRecreatePeriod; rp > 0 && ch.Spec.Type == v1.ACMEChallengeTypeDNS01 && ch.Status.State == v1.Pending &&
		...
		log.Info(fmt.Sprintf("successfully deleted stale challenge %s/%s", ch.Namespace, ch.Name))
		return nil
	}
